...
my $test_log_file= &Info::get_all_mtest_responses_file_path; 
...
if ($q->param( 'correct_answer' )) { 
	...
	unless ($q->param( 'users_answer' )){ 
		...
		print $q->header;
		print print &Encode::encode('utf-8', $tmpl->output);;
		exit; # Terminate all programs
	}
...
my $choices_answer_file = &Info::get_all_m_choices_answer_file_path;
...
my $mp3_for_test = @$mp3_all_ref[$count]; # Answer mp3
