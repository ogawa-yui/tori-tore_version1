...
my $tmpl = &Util::load_html_tmpl('17_posttest_score.tmpl' );
...
unless ($q->param( 'users_answer' )){ 
	...
	my $message = "<form method='post' action='15_posttest.cgi' > 回答してください。
...
&Quiz::write_data
  ($posttest_responses, $user, $mp3_for_test, $correct_answer, $users_answer); 

# Read the posttest results record file to determine the number of questions and the number of correct answers by the user.
my ($post_question, $post_correct) 
        = &Quiz::get_n_question_and_correct_for_test($q, $user, $posttest_responses);

$tmpl->param( post_question => "$post_question" ); 
$tmpl->param( post_correct => "$post_correct" ); 
