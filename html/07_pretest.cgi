...
my $pre_comment = "（難しいので今の時点では答えられなくても大丈夫！）";
$tmpl->param( pre_comment => $pre_comment );
...
if ($count >= $question){ 
	my $tmpl = &Util::load_html_tmpl('warning_message.tmpl' );
	my $message = "<form method='post' action='00_login_01.cgi' > すでに$test_name_jp"
				 ."腕試しを解き終わっています。
